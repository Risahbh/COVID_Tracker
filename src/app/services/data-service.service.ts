...
import { DataWiseData } from '../models/dateWiseData';
...
export class DataServiceService {
  ...
  getDateWiseData(){
    return this.http.get(this.dateWiseDataUrl, {responseType: 'text'}).pipe(map(res=>{
      let rows = res.split('\n');
      // console.log(rows);
      let mainData: any = [];
      let header = rows[0];
  ...
  getGlobalData(){
    return this.http.get(this.globalDataUrl, {responseType: 'text'}).pipe(map(res=> 
